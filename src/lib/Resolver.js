...
export default class Resolver {
    _words;
    constructor() {
        this._words = allWords.filter((w) => !w.includes('-'));
    }
    getWords(pattern, containedLetters, tries) {
        ...
        this._words = this._words
            ...
            .filter((w) => {
            if (!regexExcludes.test(w))
                return false;
            if (regexsErrors.some((e) => e.test(w)))
                return false;
            if (tries.includes(w))
                return false;
